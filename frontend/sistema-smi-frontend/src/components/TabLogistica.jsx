...
export default function TabLogistica({ productoActivo, paisesDestino, paisOrigen, onDatosActualizados }) {
  ...
  // Función centralizada para recargar datos desde Supabase
      ...
      <div className="space-y-2 pt-2">
        ...
        <div className="overflow-x-auto max-h-[380px] overflow-y-auto border border-[#1b1f2e] rounded-lg shadow-lg">
          <table className="w-full text-left text-xs text-slate-300 relative">
            ...
            <tbody className="divide-y divide-[#1b1f2e]/60 bg-[#10121b]">
              {(() => {
                const copiaOrdenada = [...datosLogisticaNormalizados].sort((a, b) => {
                  const faltantesA = [a.idlNorm, a.ccpNorm, a.ttiNorm].filter(v => v === 0).length;
                  const faltantesB = [b.idlNorm, b.ccpNorm, b.ttiNorm].filter(v => v === 0).length;
                  if (faltantesA !== faltantesB) return faltantesA - faltantesB;
                  return b.costoTotal - a.costoTotal;
                });

                return copiaOrdenada.map((row, index) => (
                  <tr key={index} className="hover:bg-[#151824] transition-colors">
                    <td className="p-3 text-slate-500">{index + 1}</td>
                    <td className="p-3 font-medium text-white">{row.Paises}</td>
                    ...
                    <td className="p-3 text-indigo-400 font-bold">{row.costoTotal}</td>
                  </tr>
                ));
              })()}
            </tbody>
