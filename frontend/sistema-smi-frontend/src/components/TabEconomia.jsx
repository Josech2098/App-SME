...
export default function TabEconomia({ productoActivo, paisesDestino, paisOrigen, datosCostoDeVida = [], onDatosActualizados }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // ================= NORMALIZACIÓN INDIVIDUAL POR COLUMNA =================
      ...
      const dfNorm = dfEcon.map(item => {
        ...
        // Verificamos si cuenta con TODOS los datos para calcular el total ponderado
            ...
            <tbody className="divide-y divide-[#1b1f2e]/60 bg-[#10121b]">
              {datosEconNormalizados.map((row, index) => (
                <tr key={index} className="hover:bg-[#151824] transition-colors">
                  ...
                  <td className="p-3">
                    {row.ICV_norm !== null ? row.ICV_norm : <span className="text-slate-600 italic">sin datos</span>}
                  </td>
                  <td className="p-3">
                    {row.INAN_norm !== null ? row.INAN_norm : <span className="text-slate-600 italic">sin datos</span>}
                  </td>
                  <td className="p-3">
